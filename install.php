...
elseif($stage==3) {
	...
	/*** test de la base ***/
	$link = mysql_connect($dbhost, $dbuser, $dbpass) or die($strConnexionImpossible);

	if(!mysql_select_db($dbname, $link))
	{
		/*** creation de la base ***/
		if(!mysql_query("CREATE DATABASE $dbname", $link)) die($strCreationImpossible." : ".mysql_error());
	}
	...
	foreach($a as $q) {
		if(trim($q)=="") break;
		//$q = str_replace("%dbprefix%", $dbprefix, $q);		
		if(!$db->query($q))
			die(mysql_error());
	} 
	$db->exec();
	...
	/* file exists? load into array 
	if(!($a = @file($sql_file))) {
		show_warning("$strFichierSqlManquant : $sql_file<br>");
		die();
	}*/
	...
	foreach($a as $i => $l) {
		$a[$i] = trim($l);
		if(substr($a[$i], 0, 1)=="#" || empty($l)) unset($a[$i]);
	}*/
	...
	echo "pensez � visiter les forum phpTournois http://forum.phptournois.net/ !";
