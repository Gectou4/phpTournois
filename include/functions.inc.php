...
if (preg_match("`functions.inc.php`i", $_SERVER['PHP_SELF'])) {
...
function show_enligne($id)
{
    ...
    //	$img="<img src=\"images/flags/$joueur->origine.gif\" border=\"0\" align=\"absmiddle\" alt=\"$joueur->origine\">";
    //	 }
    if (preg_match('`a`i', $joueur->grade) || preg_match('`ib`i', $joueur->grade)) {
        $memcolor = $mods['MODEnLigneA'];
    } elseif (preg_match('`m`i', $joueur->grade)) {
        $memcolor = $mods['MODEnLigneMo'];
    } elseif (preg_match('`n`i', $joueur->grade)) {
        $memcolor = $mods['MODEnLigneN'];
    } elseif (preg_match('`x`i', $joueur->grade)) {
        $memcolor = $mods['MODEnLigneM'];
    } elseif (preg_match('`y`i', $joueur->grade)) {
...
function show_consignes($str)
{
    ...
    echo "<table cellspacing=0 cellpadding=0 border=0 align=center style=\"margin:0 10px;\">";
...
function verif_admin_tournois($joueur, $tournois, $a = FALSE, $b = FALSE, $t = FALSE)
{
    global $PHP_SELF;

    if (empty($joueur)) //|| 	empty($tournois))
    {
        js_goto($PHP_SELF);
    }
...
function verif_manager($equipe, $joueur)
{
    ...
    if (empty($joueur) || empty($equipe) || !(equipe_manager($equipe, $joueur) || $grade['a'] != 'a' || $grade['b'] != 'b' || $grade['e'] != 'e' || $grade['j'] != 'j')) {
        js_goto($PHP_SELF);
    }
}
...
function renderBBCode(&$str)
{
    ...
    for ($i = 0; $i < count($tab_smilies); $i++) {
        $tab_smiley = preg_split('/,/', $tab_smilies[$i]);
    ...
    if ($db) {
        ...
        if ($db->num_rows($res) > 0) {
            ...
            if ((preg_match("`Nav`", getenv("HTTP_USER_AGENT"))) || (preg_match("`Gold`", getenv("HTTP_USER_AGENT"))) || (preg_match("`X11`", getenv("HTTP_USER_AGENT"))) || (preg_match("`Mozilla`", getenv("HTTP_USER_AGENT"))) || (preg_match("`Netscape`", getenv("HTTP_USER_AGENT"))) AND (!preg_match("`MSIE`", getenv("HTTP_USER_AGENT")))) {
                ...
                $browser = "Konqueror";
            } elseif ((preg_match("`bot`", getenv("HTTP_USER_AGENT"))) || (preg_match("`Google`", getenv("HTTP_USER_AGENT"))) || (preg_match("`Slurp`", getenv("HTTP_USER_AGENT"))) || (preg_match("`Scooter`", getenv("HTTP_USER_AGENT"))) || (preg_match("`Spider`", getenv("HTTP_USER_AGENT"))) || (preg_match("`Infoseek`", getenv("HTTP_USER_AGENT")))) {
...
{
    global $db, $dbprefix, $rubrique, $joueur;
