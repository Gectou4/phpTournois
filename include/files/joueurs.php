...
/*
   ...
   +---------------------------------------------------------------------+
*/
if (preg_match("/joueurs.php/i", $_SERVER['PHP_SELF'])) {
                ...
                if ($db->num_rows($joueurs) != 0) {
                    ...
                    for ($i = 0; $i < $col_joueurs; $i++) {
                        ...
                        for ($j = $i; $j < count($tab_joueurs); $j = $j + $col_joueurs) {
                            ...
                            if ($tab_joueurs[$j]->age) echo "&nbsp;$strAn";
                            echo '</td>';
                            ...
                            if ($op == 'admin') {
                                ...
                                echo '<option value="C"';
                                if ($tab_joueurs[$j]->etat == 'C') echo ' SELECTED';
                                echo ">$strCache";
                                echo '<option value="M"';
                                if ($tab_joueurs[$j]->etat == 'M') echo ' SELECTED';
                                echo ">$strAttenteMail";
                                echo '<option value="P"';
                                if ($tab_joueurs[$j]->etat == 'P') echo ' SELECTED';
                                echo ">$strPreinscrit";
                                echo '<option value="I"';
                                if ($tab_joueurs[$j]->etat == 'I') echo ' SELECTED';
                                echo ">$strInscrit";
                                echo '</select>';
...
if ($op != "inscription" && $op != "do_inscription" && $op != "inscription" && $op != "modify_fiche" && $op != "change_passwd" && $op != "do_envoi_passw" && $op != "reset_passwd") {
    if ($rech_v != "u") {
        ...
        echo '<form method="post" action="?page=joueurs&op=rechj">';
    }
    echo '<table border=0 cellpadding="0" cellspacing="0" class="bordure2"><tr><td>';
    echo '<table cellspacing="1" cellpadding="0" border="0">';
    echo "<tr><td class=\"headerfiche\">$strRechercherJoueur</td></tr>";
