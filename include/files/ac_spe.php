...
if ($_GET['op'] == "admin") {
    ...
    if ($_GET['admin'] == "add") {
        if ($_GET['up'] == "addX") {
            ...
            $reqlistaddx = $db->query($sqllistaddx) or die('Erreur SQL !<br>' . $sqllistaddx . '<br>' . $db->getError());

            while ($datalistaddx = $db->fetch_array($reqlistaddx)) {
            ...
            if (preg_match("/,/", $_POST['addprix'] || !is_numeric($_POST['addprix']))) {
                $erreur = 1;
    ...
    /*if ($_GET['admin'] == "del")
        {
         if ($_GET['up'] == "delX")
            ...
            if ($del_pseudo != "")
                ...
                $reqdelx = $db->query($sqldelx) or die('Erreur SQL !<br>'.$sqldelx.'<br>'.$db->getError());
                }
            }
    }*/
    /********************************************************
     * Traiter toute les commandes comme étant 'prisent' (allprit)
     */
    if ($_GET['allprit'] != "") {
        $lcid = $_GET['allprit'];
    ...
    /********************************************************
     *  Signaler que toute les commandes sont arrivées (allarrive)
     */
    if ($_GET['allarrive'] != "") {
        $lcid = $_GET['allarrive'];
    ...
    /********************************************************
     *  Signaler que toute les commandes sont réglées (allregle)
     */
    if ($_GET['allregle'] != "") {
        $lcid = $_GET['allregle'];
    ...
    if ($_GET['wtl'] != "") {
        ...
        $reqlistc = $db->query($sqllistc) or die('Erreur SQL !<br>' . $sqllistc . '<br>' . $db->getError());

        while ($datalistc = $db->fetch_array($reqlistc)) {
    ...
    if ($_GET['admin'] == "statuson") {
        ...
        $reqlustss = $db->query($sqllustss) or die('Erreur SQL !<br>' . $sqllustss . '<br>' . $db->getError());
        ...
        if ($_GET['annule'] != "") {
            ...
            $reqdelt = $db->query($sqldelt) or die('Erreur SQL !<br>' . $sqldelt . '<br>' . $db->getError());
        }
        ...
        if ($_GET['cid'] == "ok") {
            ...
            $reqlistc2 = $db->query($sqllistc2) or die('Erreur SQL !<br>' . $sqllistc2 . '<br>' . $db->getError());

            while ($datalistc2 = $db->fetch_array($reqlistc2)) {
                $lcid2 = $datalistc2['article_config'];
            }

            $lcid2 = $lcid2 + 1;
            $sqllustss2 = "UPDATE `${dbprefix}config` SET article_config='$lcid2'";
            $reqlustss2 = $db->query($sqllustss2) or die('Erreur SQL !<br>' . $sqllustss2 . '<br>' . $db->getError());
        }
    }
    ...
    if ($_GET['admin'] == "del") {
        ...
        $reqdell = $db->query($sqldell) or die('Erreur SQL !<br>' . $sqldell . '<br>' . $db->getError());
        $di = 0;
        while ($datadell = $db->fetch_array($reqdell)) {
            $di++;
    ...
    if ($_GET['admin'] == "list") {
        ...
        if ($_GET['ac_op'] == "regle") {
            ...
            $ac_chex_color = "green";
            $ac_w = $_GET['ac_op'];
            $ac_l = "[<font color=\"green\"><b>P</b></font>]";
        } elseif ($_GET['ac_op'] == "arrive") {
            $ac_op = "arvxp";
            $ac_chex_color = "blue";
            $ac_w = $_GET['ac_op'];
            $ac_l = "[<font color=\"blue\"><b>C</b></font>]";
        } elseif ($_GET['ac_op'] == "prit") {
            $ac_op = "prixp";
            $ac_chex_color = "navy";
            $ac_w = $_GET['ac_op'];
            $ac_l = "[<font color=\"navy\"><b>R</b></font>]";
        } elseif ($_GET['ac_op'] == "del") {
            $ac_op = "delxp";
            $ac_chex_color = "red";
            $ac_w = $_GET['ac_op'];
            $ac_l = "[<font color=\"red\"><b>A</b></font>]";
        } elseif ($_GET['ac_op'] == "") {
            $ac_op = "delxp";
            $ac_chex_color = "red";
            $ac_w = "del";
        ...
        $reqliwt = $db->query($sqlliwt) or die('Erreur SQL !<br>' . $sqlliwt . '<br>' . $db->getError());

        while ($dataliwt = $db->fetch_array($reqliwt)) {
        ...
        $reqlist = $db->query($sqllist) or die('Erreur SQL !<br>' . $sqllist . '<br>' . $db->getError());
        $pt = $db->num_rows($reqlist);

        $ilist = 0;

        while ($datalist = $db->fetch_array($reqlist)) {
            $ilist++;
        ...
        /********************************************************
         * Commande détecté.. bzzp présenter vos papier... On plaisante pas ^^
         */
    ...
    if ($_GET['ac_op'] == "rechup") {
        ...
        $ireqdel = $db->query($isqldel) or die('Erreur SQL !<br>' . $isqldel . '<br>' . $db->getError());
        ...
        $preqdel = $db->query($psqldel) or die('Erreur SQL !<br>' . $psqldel . '<br>' . $db->getError());
        ...
        $xreqdel = $db->query($xsqldel) or die('Erreur SQL !<br>' . $xsqldel . '<br>' . $db->getError());
        ...
        $rreqdel = $db->query($rsqldel) or die('Erreur SQL !<br>' . $rsqldel . '<br>' . $db->getError());
    ...
    if ($_GET['id'] != "") {
        ...
        $reqrech = $db->query($sqlrech) or die('Erreur SQL !<br>' . $sqlrech . '<br>' . $db->getError());

        while ($datarech = $db->fetch_array($reqrech)) {
