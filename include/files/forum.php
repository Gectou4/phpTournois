...
// On r&eacute;cupère les nouveaux messages
if ( $s_joueur!= "" )
{
...
if ($_GET['op']=="" || $_GET['op']==NULL) {
		...
		echo'<tr>';
		echo '<td align=center class=endforum>&nbsp;<span style="color:black;font-size:12";>'.$strCategorie.'&nbsp;</span></td>';
		echo '<td align=center class=endforum>&nbsp;<span style="color:black;font-size:12";>'.$strSujet.'&nbsp;</span></td>';
		echo '<td align=center class=endforum>&nbsp;<span style="color:black;font-size:12";>'.$strMessage.'&nbsp;</span></td>';
		echo '<td align=center class=endforum>&nbsp;<span style="color:black;font-size:12";>'.$strDernier.'&nbsp;</span></td>';
		echo '</tr>';
		...
		if ($grade['a']=='a'||$grade['b']=='b') {$is_able_to_see = 'yes';}
			
		if (eregi('m', $datar->reserved)&&$grade['m']=='m') {$is_able_to_see = 'yes';}

		if (eregi('n',$datar->reserved)&&$grade['n']=='n') {$is_able_to_see = 'yes';}

		if (eregi('u', $datar->reserved)&&$s_joueur!='') {$is_able_to_see = 'yes';}
		
		if ($is_able_to_see != 'yes' && !eregi('u', $datar->reserved) && !eregi('m', $datar->reserved) && !eregi('n', $datar->reserved)) {$is_able_to_see = 'yes';}
		
			if($datar->descri!=''||$datar->descri!=NULL){$ifcat="<br><font size=-4>&nbsp;". bbcode(stripslashes($datar->descri))."</font>";}

			if ($datar->cattitle!=''||$datar->cattitle!=NULL) {
			
			// je fait trois requête et oui c pas bien paske si je met et incr&eacute;mente des champs PHP et SQL ne veulent ni afficher C champ ni les incr&eacute;menter (c a en devenir fou Oo)
			...
			if ($datar2->edit=='O'){$date=$datar2->edit_date;}
			}
		...
		echo '<tr><td class="catfiche" colspan="4" align="center"><div align="center">'.$strFrasUnacces.'</DIV></td></tr>';
			...
			if  ($grade['a']=='a'||$grade['b']=='b') {echo '<tr><td align=right colspan="4" class=endforum><a href="?page=forum&op=addcat"><img src="themes/'.$s_theme.'/images/t_new.gif" border=0 ></a></td></tr>';}
			else {echo '<tr><td align=right colspan="4" class=endforum>&nbsp;</td></tr>';}
		echo '</table><br>';
}
/********************************************************
* Affichage TOPIC 
...
if ($_GET['op']=="topic") {
		...
		$is_able_to_see='';
		if ($grade['a']=='a'||$grade['b']=='b') {$is_able_to_see = 'yes';}
		if (eregi('m', $reserved)&&$grade['m']=='m') {$is_able_to_see = 'yes';}
		if (eregi('n',$reserved)&&$grade['n']=='n') {$is_able_to_see = 'yes';}
		if (eregi('u', $reserved)&&($grade['z']=='z')) {$is_able_to_see = 'yes';}
		if ($is_able_to_see != 'yes' && !eregi('u', $datar->reserved) && !eregi('m', $datar->reserved) && !eregi('n', $datar->reserved)) {$is_able_to_see = 'yes';}
		if ($is_able_to_see!='yes'){js_goto("?page=forum");}
		...
		if ($desc=="O") {$desct="DESC";}
		else if ($desc==""||$desc==NULL) {$desct="DESC";}
		else {$desct="";}	
...
if ($_GET['op']=="read") {
		...
		if ($grade['a']=='a'||$grade['b']=='b') {$is_able_to_see = 'yes';}
		if (eregi('m', $reserved)&&$grade['m']=='m') {$is_able_to_see = 'yes';}
		if (eregi('n',$reserved)) {$is_able_to_see = 'yes';}
		if (eregi('u', $reserved)&&($grade['z']=='z')) {$is_able_to_see = 'yes';}
		if ($is_able_to_see != 'yes' && !eregi('u', $datar->reserved) && !eregi('m', $datar->reserved) && !eregi('n', $datar->reserved)) {$is_able_to_see = 'yes';}
		if ($is_able_to_see!='yes'){js_goto("?page=forum");}
		...
		if (!$topic || !$cat || !$titre || !$topid) {js_goto("?page=forum");}
		...
		while ($datar = $db->fetch($resr)) {
			...
			for($i=0;$i<count($tab_rank);$i++) {
				$tab_rank_split=split(',',$tab_rank[$i]);
...
if ($_GET['op']=="addreply") {
			...
			echo "<tr>";
			echo "<td class=textfiche align=center width='450'>";buttonBB('contenu');echo "</td>";
			echo "</tr>";
			echo "<tr>";
...
if ($_GET['op']=="add_topic") {
	...
	if($erreur==1) {
		...
		while ($datar = $db->fetch($resr)) {$topid=$datar->topid;}
		$topid=$topid+1;
		$date=time();
...
if ($_GET['op']=="deletereply") {
	...
	if ($grade['a']!='a'||$grade['b']!='b'||$grade['m']!='m'){js_goto("?page=index.php");} 
...
if ($_GET['op']=="editreply") {
		...
		while ($datar = $db->fetch($resr)) {
			...
			echo "<table cellspacing=0 cellpadding=3 border=0 width=100%>";
			echo "<tr>";
			echo "<td class=textfiche align=center>";buttonBB('contenu');echo "</td>";
			echo "</tr>";
			echo "<tr>";
			//echo "<td class=titlefiche style=\"white-space:normal\">$strContenu :</td>";
...
if ($_GET['op']=="add_cat") {
	...
	if($erreur==1) {
		...
		while ($datar = $db->fetch($resr)) {$cattopic=$datar->cattopic;}
		$cattopic=$cattopic+1;
