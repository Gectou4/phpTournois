...
if($mode=='J') {
    ...
    if ( ($grade['a']!='a'&&$grade['b']!='b'&&$grade['j']!='j') && ($grade['z']=='' || $id != $s_joueur) ) { js_goto("?page=index"); } 
	
	/*** recup&eacute;ration des paramètres ***/
	$ficheX = joueur($id);	
	$type = "joueurs";
}
elseif($mode=='E') {
    ...
    if ( ($grade['a']!='a'||$grade['b']!='b'||$grade['j']!='j') && ($grade['z']=='' || !equipe_manager($id,$s_joueur) )) { js_goto("?page=index"); } 
