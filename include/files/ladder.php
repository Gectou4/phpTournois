...
if (preg_match("/ladder.php/i", $_SERVER['PHP_SELF'])) {
...
if ($ad == "ad" && ($grade['a'] != 'a' && $grade['b'] != 'b' && $grade['u'] != 'u')) {
    js_goto('?page=login');
}
//
// Ajout d'un ladder
...
<select name="a">';
    ...
    if ($lad_mod->ladder_type == "1") {
        echo "<option value=\"1\" selected>$strJoueur</option>";
    } else if ($lad_mod->ladder_type == "2") {
        echo "<option value=\"2\" selected>$strEquipe</option>";
    }
    echo '
					<option value="1">' . $strJoueur . '</option>
					<option value="2">' . $strEquipe . '</option>
    ...
    ////
    if ($lad_mod->s_frag == 1) {
        $sfrag = 'checked="checked"';
        $disablefrag = '';
    } else {
        $sfrag = '';
        $disablefrag = 'disabled';
    }
    if ($lad_mod->s_round == 1) {
        $sround = 'checked="checked"';
        $disableround = '';
    } else {
        $sround = '';
        $disableround = 'disabled';
    }
    if ($lad_mod->s_manche == 1) {
        $smanche = 'checked="checked"';
        $disablemanche = '';
    } else {
        $smanche = '';
        $disablemanche = 'disabled';
    }
    echo '
	<tr><td>
	<table cellspacing="0" cellpadding="2" border="0" width="100%">
...
<select name="a">';
    ...
    $lad_while = $db->fetch();
    $map_array = array();
    $map_array = preg_split('/,/', $lad_while->maps);
...
//
    ...
    $lad_map = $db->fetch();
    $map_array = array();
    $map_array = preg_split('/,/', $lad_map->maps);
...
//
    ...
    while ($lad_while = $db->fetch($res)) {
        ...
        if ($lad_while->death == '' || $lad_while->death == 0 || $lad_while->death == '0') {
            $div = '1';
        } else {
            $div = $lad_while->death;
        }
...
//
    ...
    if ($lad_while->reglement != '' || $lad_while->reglement != NULL) {
        $lad_test = "1";
    }
    ...
    if ($lad_while->reglement != "") {
        echo BBcode($lad_while->reglement);
    } else {
        echo '- ' . $strLAD_nodata_rule . ' - ';
    }
...
//
    ...
    if ($lad_while->rapport != '' || $lad_while->rapport != NULL) {
        $lad_test = "1";
    }
    ...
    echo '</td>
							...
							<tr class="ladder_line" bgcolor="#3c3c46">
								...
								';
    echo nl2br($lad_while->rapport);
    echo '</center><br /><br /></td>
								  </td></tr></table>
...
//
    ...
    if ($nb_manche == '0' || $nb_manche == 0 || $nb_manche == NULL || $nb_manche == '') {
        $nb_manche = 1;
    }
    ...
    if (!empty($disagree)) {
        ...
        $save_j1_split = array();
        $save_j1_split = preg_split('/!/', $lad_while->save_j1);

        for ($i = 0; $i < count($save_j1_split); $i++) {

            $save_j1_split[$i] = preg_split('/,/', $save_j1_split[$i]);
    ...
    if ($map_array[0] == "/") {
        ...
        $map_array = array();
        $map_array = preg_split('/,/', $lad_while_m->maps);
    }
    $j2 = $lad_while->j2;
    echo '
        ...
        if ($hide_form == false) {
            ...
            if (((($lad_while->valide == 'A' && $s_joueur == $lad_while->j1) || ($lad_while->valide == 'X' && $s_joueur == $lad_while->j2)) && ($ad != 'ad')) || $s_joueur == '') {
                ...
                if ($s_joueur == '') {
                    ...
                    $strLAD_you_must_be_wait_txt = $strLAD_you_must_be_wait;
                }
                echo '
...
//if (!empty($ad) AND ($garde['a']!='a' && $garde['b']!='b' && $garde['u']!='u')){js_goto('?page=INDEX');}
    ...
    if (!empty($agree)) {
        ...
        for ($i = 0; $i < count($save_split); $i++) {
            ...
            $save_split[$i] = preg_split('/,/', $save_split[$i]);
        ...
        for ($i = 1; $i < $nb_manche + 1; $i++) {
            ...
            if ($t_total_manche_j2_n == 0) {
                $t_total_manche_j2_n = '0';
            }
            if ($t_total_manche_j1_n == 0) {
                $t_total_manche_j1_n = '0';
            }
            if ($t_total_manche_j2_w == 0) {
                $t_total_manche_j2_w = '0';
            }
            if ($t_total_manche_j2_l == 0) {
                $t_total_manche_j2_l = '0';
            }
            if ($t_total_manche_j1_w == 0) {
                $t_total_manche_j1_w = '0';
            }
            if ($t_total_manche_j1_l == 0) {
                $t_total_manche_j1_l = '0';
            }
...
//
    ...
    while ($lad_while = $db->fetch($res)) {
        ...
        if ($lad_while->ladder_name != '' || $lad_while->ladder_name != NULL) {
            $lad_test = "1";
        }
