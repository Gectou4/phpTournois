...
/*
   ...
   +---------------------------------------------------------------------+
*/
if (eregi("maps.php", $_SERVER['PHP_SELF'])) {
	...
	if ($db->num_rows($maps) != 0) {
		...
		for($i=0;$i<$config['col_maps'];$i++) {
			...
			for($j=$i;$j<count($tab_maps);$j=$j+$config['col_maps']) {
				echo "<tr>";
				echo "<td class=textliste>";if($tab_maps[$j]->icone) echo "<img src=\"images/jeux/".$tab_maps[$j]->icone."\" align=absmiddle> ";echo $tab_maps[$j]->sigle."</td>";
