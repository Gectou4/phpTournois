...
/*
   ...
   +---------------------------------------------------------------------+
*/
if (eregi("matchs_gestion.php", $_SERVER['PHP_SELF'])) {
	...
	if(!empty($id)) {
		...
		if($modeequipe_tournois=='E') {
			...
			if($config['avatar']=='E' || $config['avatar']=='A') {
				$avatar_ok=1;
				$equipe1=equipe($match->equipe1);$img1=$equipe1->avatar_img;
				$equipe2=equipe($match->equipe2);$img2=$equipe2->avatar_img;				
			}				
		}
		else {
			$show="show_joueur";
			...
			if($config['avatar']=='J' || $config['avatar']=='A') {
				$avatar_ok=1;
				$joueur1=joueur($match->equipe1);$img1=$joueur1->avatar_img;
				$joueur2=joueur($match->equipe2);$img2=$joueur2->avatar_img;				
			}
		}
		...
		if($avatar_ok && $op!='admin') {
			echo "<tr>";
			echo "<td class=text colspan=1 align=center>";echo show_avatar($img1);echo "</td>";
			echo "<td class=text colspan=3 align=center><font size=+2>$strVS</font></td>";
			echo "<td class=text colspan=1 align=center>";echo show_avatar($img2);echo "</td>";
			echo "</tr>";
		}
		echo "<tr>";
		...
		if($op=='admin' || $report_type==1) {
			...
			echo "<input type=checkbox name=statusequipe value=F1 ";if ($match->statusequipe == "F1") echo " CHECKED";echo " onclick=\"oneOrNoCheckboxGroup(this);\" style=\"border: 0px;background-color:transparent;\">$strForfait&nbsp;&nbsp;";
			echo "<input type=checkbox name=statusequipe value=D1 ";if ($match->statusequipe == "D1") echo " CHECKED";echo " onclick=\"oneOrNoCheckboxGroup(this);\" style=\"border: 0px;background-color:transparent;\">$strDisqualifie";
			echo "</td>";
			echo "<td class=text></td>";
			echo "<td class=text colspan=2 align=right>";
			echo "$strDisqualifie<input type=checkbox name=statusequipe value=D2 ";if ($match->statusequipe == "D2") echo " CHECKED";echo " onclick=\"oneOrNoCheckboxGroup(this);\" style=\"border: 0px;background-color:transparent;\">&nbsp;&nbsp;";
			echo "$strForfait<input type=checkbox name=statusequipe value=F2 ";if ($match->statusequipe == "F2") echo " CHECKED";echo " onclick=\"oneOrNoCheckboxGroup(this);\" style=\"border: 0px;background-color:transparent;\">";
			echo "</td>";
			echo "</tr>";
		}
		...
		for($i=1;$i<=$db->num_rows($manches)+1;$i++) {
			...
			echo "<input type=checkbox name=mancheencours value=$manche->id";if($manche->status == "A") echo " CHECKED";if($op!='admin') echo " DISABLED";echo " onclick=\"oneOrNoCheckboxGroup(this);\" style=\"border: 0px;background-color:transparent;\">";
			echo "</td>";
		...
		if ($op == 'admin' || $match->serveur) {
			...
			if($op == 'admin') {
				...
				while($serveur = $db->fetch($serveurs)) {
					...
					echo "<option value='$serveur->id'";if($serveur->id == $match->serveur) echo " SELECTED";echo ">#$serveur->id - $servername";
				}
				echo "</select>";
		...
		echo "<td class=textfiche>";
		if($op=='admin') {
			echo "<input type=radio name=status value=C";if($match->status == "C") echo " CHECKED";echo " style=\"border: 0px;background-color:transparent;\"> $strCache";
			echo "<input type=radio name=status value=A";if($match->status == "A") echo " CHECKED";echo " style=\"border: 0px;background-color:transparent;\"> $strActif";
			echo "<input type=radio name=status value=D";if($match->status == "D") echo " CHECKED";echo " style=\"border: 0px;background-color:transparent;\"> $strEnCours";			
			echo "<input type=radio name=status value=T";if($match->status == "T") echo " CHECKED";echo " style=\"border: 0px;background-color:transparent;\"> $strTermine";
		}
		else {
