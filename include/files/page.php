...
if (eregi("page", $_SERVER['PHP_SELF'])) {
...
if($op == "addpage") {
	...
	if ($grade['a']!='a'&&$grade['b']!='b'&&$grade['g']!='g') {js_goto($PHP_SELF);} 
	...
	if($erreur==1) {		
		...
		if($mods['pagescript']=='0'||$mods['pagescript']==''||$mods['pagescript']==NULL){$contenu = remove_XSS($contenu);}
	...
	$id=$_GET['id'];
		...
		$sqlx = "SELECT titre FROM ${dbprefix}menu"; 
  		$reqx = mysql_query($sqlx) or die('Erreur SQL !<br>'.$sqlx.'<br>'.mysql_error());				
			...
			if ($data['acces']=="0") {$rubrique="$strTous";}
			if ($data['acces']=="A") {$rubrique="$strAdmins";}
			if ($data['acces']=="N") {$rubrique="$strNewseur";}
			if ($data['acces']=="P") {$rubrique="$strPremium";}
			if ($data['acces']=="M") {$rubrique="$strjoueurs";}
 ...
 */
 if ($_GET['op']=="see") {
		...
		while($data = mysql_fetch_array($req)) 
					...
					if ($acces=="A") {if ($grade['a']!='a'&&$grade['b']!='b'&&$grade['g']!='g') js_goto('?page=login');}//admin
					if ($acces=="N") {if ($grade['a']!='a'&&$grade['b']!='b'&&$grade['g']!='g'&&$grade['n']!='n') js_goto('?page=login');}//Newser
					if ($acces=="M") {if($s_joueur=="" || $s_joueur==NULL) js_goto('?page=login');}//joueur
					if ($acces=="P") {if(!premium($s_joueur) && ($grade['a']!='a'&&$grade['b']!='b'&&$grade['g']!='g')) js_goto('?page=login');}//premium
			
				if($grade['a']=='a'||$grade['b']=='b'||$grade['g']=='g') {echo "<a href='?page=page&op=modif&id=".$data['id']."'><img src='images/edit.gif' border=0 /></a><a href='?page=page&op=delpage&id=".$data['id']."'> <img src='images/f.gif' border=0 /></a><br><br>";}
				$contenu=BBcode($contenu);
				$contenu=stripslashes($contenu);
				$contenu=wordwrap($contenu, 180, "\n", 1);
