...
/*
   ...
   +---------------------------------------------------------------------+
*/
if (preg_match("/equipes.php/i", $_SERVER['PHP_SELF'])) {
...
elseif ($op == "add_joueur") {
    ...
    if ($erreur == 1) {
        ...
        if (equipe_appartient($id, $joueur, $jeux)) {
            ...
            if (!preg_match("/y/i", $joueur_res->grade) && ($status == "1" || $status == "2")) {
...
elseif ($op == "delete_joueur") {
    ...
    if ($erreur == 1) {
        ...
        if ($coun_y_grade <= 1) {
            ...
            if (preg_match("/y/i", $joueur_ress->grade)) {
...
elseif ($op == "modify_manager") {
    ...
    if ($erreur == 1) {
        ...
        if (!preg_match("/x/i", $joueur_x->grade)) {
...
elseif ($op == "modify_admin") {
    ...
    if (!$email || !preg_match("/i^[a-z0-9\._-]+@+[a-z0-9\._-]+\.+[a-z]{2,3}$/i", $email)) {
        $erreur = 1;
        $str .= "- $strElementsEmailInvalide<br>";
    }
...
elseif ($op == "edit_stat") {
    ...
    echo '>' . $strLeader . '</option>
	<option value="2"';
    if ($stat_j == "2") {
        echo 'selected';
    }
    echo '>' . $strWarArranger . '</option>
	<option value="3"';
    if ($stat_j == "3") {
        echo 'selected';
    }
    echo '>' . $strMembre . '</option>
	<option value="4"';
    if ($stat_j == "4") {
        echo 'selected';
    }
    echo '>' . $strRecrue . '</option>
	<option value="5"';
    if ($stat_j == "5") {
        echo 'selected';
    }
    echo '>' . $strInactif . '</option>
	</select></td></tr>
                    ...
                    for ($i = 0; $i < $col_equipes; $i++) {
                        ...
                        for ($j = $i; $j < count($tab_equipes); $j = $j + $col_equipes) {
                            ...
                            if ($op == 'admin') {
                                ...
                                echo '<option value="C"';
                                if ($tab_equipes[$j]->etat == 'C') echo ' SELECTED';
                                echo ">$strCachee";
                                echo '<option value="A"';
                                if ($tab_equipes[$j]->etat == 'A') echo ' SELECTED';
                                echo ">$strEnAttente";
                                echo '<option value="V"';
                                if ($tab_equipes[$j]->etat == 'V') echo ' SELECTED';
                                echo ">$strValidee";
                                echo '</select>';
