 ...
 */
	...
	//remplacer par le nom du tournois
	$export .= "<p class=title>.:: $strResultatsFinales ::.</p>";
...
//	$export .= " nb_finales_winner : $nb_finales_winner|";
	...
	$export .= '<form name="finales" method="post" action="?page=matchs_gestion">';
	$export .= "<input type=\"hidden\" name=\"x\" value=\"$x\">";
	...
	$export .= '</tr>';
	$export .= '<tr><td align="center" colspan="'.$nb_col.'">&nbsp;</td></tr>';
	...
	for ($e = 1 ; $e <= $nb_row ; $e++) {
		...
		//if($e==$nb_row_winner+1) $export .= "<tr colspan='".($nb_col+2)."'><td background=images/arbre_ligneH.gif align=center><img src=images/arbre_ligneH.gif></td></tr>";
			...
			if($e<=$nb_row_winner) {
				...
				if(($e) % pow(2, $f) == pow(2, $f-1)) {
					...
					if ($finale >= $last_finale) {
						...
						$export .= '<table cellspacing="0" cellpadding="0" border="0" width="100%">';
						$export .= '<tr><td class="info" align="center">';						
    ...
    if(!$fd = @fopen("include/files/test.php","w")) {
		$erreur=1;
		show_erreur("$strOuvertureInvalideConfigFile : $filenamem4");
	}
	elseif(!fputs($fd,  $export)) {
		$erreur=1;
