...
if($op == "add_news") {
	...
	if ($grade['a']!='a'&&$grade['b']!='b'&&$grade['n']!='n') {js_goto('?page=login');} 
...
elseif($op == "modify_news") {
	...
	if(is_numeric($id)) {
		...
		$selected=0;
			...
			if ($news->icone!=''){$g_icon1=$news->icone;}else{$g_icon1="aucun.gif";}
			...
			if ($news->icone2!=''){$g_icon2=$news->icone2;}else{$g_icon2="aucun.gif";}
		...
		//echo "<td class=textfiche><textarea cols=80 rows=10 name=contenu wrap=virtual ONSELECT=\"storeCaret(this);\" ONCLICK=\"storeCaret(this);\" ONKEYUP=\"storeCaret(this);\">".stripslashes($news->contenu)."</textarea></td>";
		/*echo '<td class=textfiche>
				<script type="text/javascript">
...
elseif($op == "imprimer") {
	...
	if($erreur==1) {			
		...
		echo "<tr><td>";
		if ($news->icone2!="") {echo "<IMG src='images/news/$news->icone' align='RIGHT' border='0'>";}
 ...
 */
 elseif($op == "do_envoyer") {
	...
	if(!$email_from || !eregi("^[a-z0-9\._-]+@+[a-z0-9\._-]+\.+[a-z]{2,3}$", $email_from)) {
 		$erreur=1;
		$str.="- 1e ".$strElementsEmailInvalide."<br>";
	}
	if(!$email_to || !eregi("^[a-z0-9\._-]+@+[a-z0-9\._-]+\.+[a-z]{2,3}$", $email_to)) {
 		$erreur=1;
 ...
 */
	...
	if ($grade['a']=='a'||$grade['b']=='b'||$grade['n']=='n') { 
	 echo "<p class=title>.:: $strAdminNews ::.</p>";}
	else {echo "<p class=title>.:: $strNews ::.</p>";}
	...
	if (empty($id)) {
		...
		if ($db->num_rows($res)==0) {
			...
			while ($news = $db->fetch($res)) {
				...
				echo "<tr>";
				echo "<td class=text>";
					if ($news->icone2!="") {echo "<IMG src='images/gicon/$news->icone2' align='RIGHT' border='0'>";}
					echo $contenu;
	...
	/*** fiche d'une news ***/
		...
		if(empty($s_joueur)) {
			...
			echo "<tr>";
			echo "<td class=textfiche colspan=2 align=center width='450'>";buttonBB('contenu');echo "</td>";
			echo "</tr>";
			echo "<tr>";
			echo "<td class=titlefiche></td>";
