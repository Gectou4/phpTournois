		...
		if ($db->num_rows($res) != 0) {
			...
			for($i=0;$i<$config['col_'.$equipesX];$i++) {
				...
				for($j=$i;$j<count($tab_participants);$j=$j+$config['col_'.$equipesX]) {
					...
					echo "<option value=P";if ($tab_participants[$j]->status == "P") echo " SELECTED";echo ">$strParticipe";
					echo "<option value=F";if ($tab_participants[$j]->status == "F") echo " SELECTED";echo ">$strForfait";
					echo "<option value=D";if ($tab_participants[$j]->status == "D") echo " SELECTED";echo ">$strDisqualifie";
					echo "</select>";					
					echo "</td>";
					echo "</tr>";
				}
